...
BEGIN
  ...
  IF b.block_id IS NOT NULL THEN
    ...
    SET company_id = b.from_company_id, 
    ...
    WHERE x >= original_block.x - 2 AND x <= original_block.x + 2 
      ...
      AND id != b.block_id;
  ELSE
    UPDATE public.blocks SET company_id = b.from_company_id, purchased_at = NOW() WHERE company_id = b.to_brand_id;
  END IF;
...
pool.query(sql)
  .then(() => {
    console.log('RPC accept_position_bid updated with purchased_at logic');
    pool.end();
  })
  .catch(err => {
    console.error(err);
